...
var myDictionary = {
    ...
    // resets the columns to display the divisions of the conference. 
...
var teamsArray = [];
var responseExt;
// first ajax call to get the team. 
$.ajax({
    url: queryURL,
    method: "GET"
    ...
    // setting the team logos to an array. Uses the id of the team to match the image to it. 
...
$(document).on("click", ".team-btn", function() {
    ...
    var urlTeam = "https://cors-anywhere.herokuapp.com/https://api.sportradar.us/nfl-t1/teams/" + teamAbrv + "/roster.json?api_key=b2uf4274y4r7ywx89fmph5e2"
        // 2nd ajax call, using data from the first to get the team information.
    $.ajax({
        url: urlTeam,
        method: "GET"
        ...
        var coach = [responseTeam.coaches[0].name_full, responseTeam.coaches[1].name_full];
        var myImg = `<img class="team-logos-big" src=${myDictionary[responseTeam.id]}>`
        var count = 0;

        var roster = responseTeam.players;

        var information = $(`<div class="row" id="team-description"><div class="col-sm-3 offset-sm-2">${myImg}</div><div class="col-sm-4 offset-sm-2" id="team-col"><div id="clicked-team">${teamCity} ${teamName} <div id="coach">Head Coaches: ${coach[0]}, ${coach[1]}</div></div></div>`)
        ...
        while (count < roster.length) {
            ...
            var displayPlayers = $(`
                <section class="columns small-4 player-col">
                    <div>
